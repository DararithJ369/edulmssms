...
@lesson_router.post("", response_model=LessonResponse, dependencies=[Depends(PermissionGuard.admin_or_teacher)])
def create_lesson(payload: LessonCreate, db: Session = Depends(get_db)):
    return LessonService.create_lesson(db, payload)


@lesson_router.put("/{lesson_id}", response_model=LessonResponse, dependencies=[Depends(PermissionGuard.admin_or_teacher)])
...
@lesson_router.delete("/{lesson_id}", dependencies=[Depends(PermissionGuard.admin_or_teacher)])
...
@lesson_router.post("/{lesson_id}/materials", dependencies=[Depends(PermissionGuard.admin_or_teacher)])
def add_material(
    lesson_id: int,
    title: str = Form(...),
...
@lesson_router.delete("/materials/{material_id}", dependencies=[Depends(PermissionGuard.admin_or_teacher)])
