...
export default function TeacherFilterSort() {
  ...
  const buttonStyles =
    "w-8 h-8 flex items-center justify-center rounded-xl bg-background border border-border/80 hover:bg-accent text-muted-foreground/80 transition-colors relative";
  ...
  return (
    <div className="flex items-center gap-2 select-none">
      ...
      <div className="relative" ref={filterRef}>
        <button
          className={`${buttonStyles} ${activeDepartment ? "border-[#0038A8]/40 bg-[#0038A8]/5 text-[#0038A8]" : ""}`}
          onClick={() => {
        ...
        {isFilterOpen && (
          <div className="absolute right-0 top-10 mt-1 w-56 bg-white dark:bg-muted border border-border/80 rounded-xl shadow-lg z-50 p-2 animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            <div className="space-y-0.5">
              {departments.map((dept) => {
                const isSelected = activeDepartment === dept.value;
                return (
                  <button
                    key={dept.label}
                    onClick={() => handleFilterChange(dept.value)}
