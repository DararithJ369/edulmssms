...
export default function AssignmentSubmissionPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (submission && submission.status === "graded") {
      setMessage({ type: "error", text: "You cannot resubmit homework that has already been graded by your instructor." });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: "success", text: "Homework submitted successfully! You can edit this submission before grading if needed." });
      setSelectedFile(null);
      setSubmitting(false);
    } catch (err: any) {
      console.error(err);
      const errMsg = err.response?.data?.detail || "An error occurred while uploading your homework.";
    ...
    <div className="flex-1 p-6 space-y-6 bg-[#F7F8FA] min-h-screen relative font-sans text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {message && (
            <div className={`p-4 rounded-xl text-sm font-semibold flex items-center gap-2 select-none ${
            ...
            {submission?.status === "graded" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full md:w-auto px-6 py-2.5 bg-[#0038A8] text-white hover:bg-[#002e8c] disabled:bg-muted disabled:text-muted-foreground font-extrabold text-xs rounded-xl shadow-md transition-all active:scale-[0.98]"
                >
