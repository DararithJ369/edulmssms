...
export default function InstructorsPage() {
  ...
  const fetchInstructors = useCallback(async () => {
    try {
      setLoading(true);
      const { data } = await api.get(`/users/instructors?page=${page}&limit=10`);

      if (data.data) {
        const instructorsWithProfiles = await Promise.all(
          data.data.map(async (instructor: Instructor) => {
            try {
              const profileRes = await api.get(`/profiles/${instructor.id}`);
  ...
  const filteredInstructors = instructors.filter(
    (instructor) =>
      (instructor.email?.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
        instructor.username?.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
        instructor.full_name?.toLowerCase().includes(debouncedSearch.toLowerCase())) &&
      (departmentFilter === "" || instructor.department === departmentFilter)
  );
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-gray-600 dark:text-gray-300 font-medium mt-2">
                Manage and view instructor directory and course assignments
              </p>
            </div>
            <AddInstructorDialog onInstructorAdded={fetchInstructors} />
          </div>
        </div>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            >
              ...
              {Array.from(new Set(instructors.map((i) => i.department))).filter(Boolean).map((dept) => (
          ...
          <Card className="bg-white/80 dark:bg-slate-800/80 backdrop-blur border-0 shadow-md dark:border dark:border-slate-700">
            <CardContent className="pt-12 pb-12">
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-gray-600 dark:text-gray-400 font-medium">Loading instructors...</p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl">
            <div className="bg-gradient-to-r from-green-50 dark:from-slate-800 to-emerald-50 dark:to-slate-700 border-b border-gray-200 dark:border-slate-700 py-4 px-6">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">Instructor Directory</h2>
                <div className="px-3 py-1 bg-[#72e3ad]/20 dark:bg-[#006239]/30 rounded-full">
                  <span className="text-xs font-bold text-[#1d7e59] dark:text-[#72e3ad]">{filteredInstructors.length} instructors</span>
                </div>
              </div>
            </div>
            <Table className="w-full">
              <TableHeader>
                <TableRow className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700">
                  ...
                  <TableHead className="font-black text-gray-900 dark:text-white text-xs uppercase tracking-wider px-6">Position</TableHead>
              ...
              <TableBody>
                {filteredInstructors.map((instructor, index) => {
                  const initials = (instructor.full_name || instructor.username || "I")
                    .split(" ")
                    .map((n) => n[0])
                    .join("")
                    .toUpperCase()
                  ...
                  return (
                    ...
                    >
                      <TableCell className="font-bold text-gray-900 dark:text-white max-w-xs truncate px-6">
                        <div className="flex items-center gap-2.5">
                          <ProfileAvatar
                            ...
                            alt={instructor.full_name || instructor.username || "Instructor"}
                            fallback={initials}
                            size="sm"
                          />
                      ...
                      <TableCell className="text-gray-600 dark:text-gray-400 max-w-xs truncate text-sm px-6">{instructor.email}</TableCell>
                      <TableCell className="text-gray-600 dark:text-gray-400 max-w-xs truncate text-sm px-6">{instructor.position || "-"}</TableCell>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => {
                                  navigate(`/users/instructors/${instructor.id}`);
                                }}
                              >
